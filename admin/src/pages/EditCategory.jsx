...
const EditCategory = () => {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${backend_url}/api/category/get/${id}`);
    ...
    fetchCategory();
  }, [id, backend_url, setValue, navigate]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Add flag to indicate if we should keep existing image
      if (!imageFile && existingImage) {
        formData.append('keepExistingImage', 'true');
      }

      await axios.put(
        `${backend_url}/api/category/edit/${id}`,
        formData,
        {
          headers: {

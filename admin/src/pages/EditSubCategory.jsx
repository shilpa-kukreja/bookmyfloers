...
const EditSubCategory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const categoriesResponse = await axios.post(`${backend_url}/api/category/all`);
        setCategories(categoriesResponse.data.data || []);
        
        // Fetch subcategory data
        const subcategoryResponse = await axios.get(`${backend_url}/api/subcategory/get/${id}`);
        const subcategory = subcategoryResponse.data.data;
        
        // Set form values
        setValue('name', subcategory.name);
        setValue('slug', subcategory.slug);
        setSelectedCategory(subcategory.categoryId); // Set the selected category ID
        
        // Set image preview
        if (subcategory.image) {
          setImagePreview(`${backend_url}${subcategory.image}`);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await axios.put(
        `${backend_url}/api/subcategory/edit/${id}`,
        formData,
  ...
  return (
    <div className="mx-auto p-6">
      <div className="flex items-center justify-between mb-8 border-b border-gray-200 py-3">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Edit Sub Category</h1>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              {!selectedCategory && (
                <p className="mt-2 text-sm text-red-600">Parent category is required</p>
              )}
            </div>
              ...
              <div className="flex-1 space-y-4">
                <div className="relative">
                  ...
                  >
                    ...
                    {imageFile ? 'Change Image' : existingImage ? 'Change Image' : 'Upload Image'}
                  </label>
                </div>
                ...
                <div className="text-sm text-gray-500">
                  <p>Upload a high-quality image for your subcategory.</p>
                  <p>Recommended size: 800x800px, JPG or PNG format, max 2MB.</p>
                  {existingImage && !imageFile && (
                    <p className="text-blue-600 mt-2">Note: Existing image will be kept if no new image is uploaded.</p>
                  )}
                </div>
              </div>
            </div>

...
const EditBlog = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${backend_url}/api/blog/get/${id}`);
        ...
        // Set image preview
        if (blog.image) {
          setImagePreview(`${backend_url}${blog.image}`);
          setOriginalImage(blog.image.split('/').pop()); // Extract filename
        }
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (imageFile) {
        ...
        formData.append('existingImage', originalImage);
      }

      await axios.put(
        `${backend_url}/api/blog/edit/${id}`,
        formData,
        {
          headers: {
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="text-sm text-gray-500">
                ...
                {originalImage && !imageFile && (
                  <p className="text-blue-600 mt-2">Current image will be kept if no new image is selected.</p>
                )}
              </div>
            </div>
          </div>
